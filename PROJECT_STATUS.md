...
**Last reconciled:** April 26, 2026 (Phase 9 — public marketing refresh COMPLETE — all 14 sub-phases shipped; Lease Phase 07 — itemized financials + utility matrix COMPLETE — all 3 chunks shipped; Lease Phase 08 — condition reports COMPLETE — all chunks shipped: DB foundation + record-inspection edge fn + tenant mobile wizard at /tenant/inspection + landlord side-by-side review at /landlord/inspection-review + admin index at /admin/inspections + 9-state move-in warning banner (CA GA KY MD MA NH NJ VA WA — fires 7d after move-in if no inspection on file) + tenant portal CTA after lease execution; **Lease Phase 09 — security deposit accounting + per-state deduction-letter generator COMPLETE — all 5 chunks shipped: DB foundation (lease_deposit_accountings + lease_deposit_deductions tables + RLS) + ~530-LOC pdf-lib renderer + admin generate-deposit-letter edge fn (dry-run + commit modes; auto-snapshots state_return_deadline from state_lease_law and flags late_generated) + 10-state letter partials (CA FL MA NJ TX NY IL GA OH MI) seeded into lease_template_partials with each state's specific demand-for-payment notice and statutory damages language + admin index at /admin/deposit-accounting (deduction editor + dry-run preview + finalize-and-download) + tenant submit-deposit-dispute edge fn + tenant view at /tenant/deposit (late banner, itemized deductions, signed-URL PDF download, dispute modal) + auto-email tenant via existing Resend/GAS/Gmail fallback chain on letter finalize**. Next: Phase 10 — leases as first-class entity (refactor).)
