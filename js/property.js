...
/* ─────────────────────────────────────────────────────────────────────────────
   Admin tools bar — visible only to signed-in admins.
   Lets an admin permanently delete a property directly from its public page.
   Uses a strict, explicit confirm() popup; deletion is irreversible (all
   linked photos / inquiries / saves cascade-delete; applications are kept
   with property_id set NULL by FK rule).
   ───────────────────────────────────────────────────────────────────────── */
(function initAdminBar(){
  const bar = document.getElementById('adminPropBar');
  if(!bar) return;
  ...
  async function isAdminUser(){
    try {
      ...
      if(!uid) return false;
      const { data } = await supabase.from('admin_roles').select('id').eq('user_id', uid).maybeSingle();
      return !!data;
  ...
  async function show(){
    ...
    const title = currentProperty.title || 'this property';
    const status = currentProperty.status || 'unknown';

    document.getElementById('adminPropStatus').textContent = '· status: ' + status;
    const editLink = document.getElementById('adminPropEdit');
    editLink.href = '/admin/properties.html?edit=' + encodeURIComponent(id);

    const delBtn = document.getElementById('adminPropDelete');
    delBtn.addEventListener('click', async () => {
      const msg = 'Permanently delete "' + title + '"?\n\n'
                + 'This CANNOT be undone. All linked photos, inquiries, and saves will be removed. '
                + 'Tenant applications tied to this property will keep their history but lose the link to the property.\n\n'
                + 'Type the property title exactly to confirm:';
      const typed = window.prompt(msg, '');
      if(typed === null) return; // cancelled
      if(typed.trim() !== title.trim()){
        alert('Title did not match — delete cancelled.');
        return;
      }
      ...
      try {
        ...
        delBtn.innerHTML = '<i class="fas fa-trash" aria-hidden="true"></i> Delete forever';
      }
    });
  ...
  // Wait until renderProperty has set currentProperty (poll briefly).
  let tries = 0;
  const tick = () => {
    if(window.currentProperty || (typeof currentProperty !== 'undefined' && currentProperty)){
      show();
    } else if(tries++ < 60){
      setTimeout(tick, 200);
    }
