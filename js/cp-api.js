...
const Properties = {
  ...
  async getListings(filters = {}) {
    ...
    // Geospatial bounds
    if (filters.min_lat !== undefined) q = q.gte('lat', parseFloat(filters.min_lat));
    if (filters.max_lat !== undefined) q = q.lte('lat', parseFloat(filters.max_lat));
    if (filters.min_lng !== undefined) q = q.gte('lng', parseFloat(filters.min_lng));
    if (filters.max_lng !== undefined) q = q.lte('lng', parseFloat(filters.max_lng));
...
function buildApplyURL(property) {
  ...
  try {
    sessionStorage.setItem('cp_property_context', JSON.stringify({
      ...
      application_fee:  property.application_fee  != null ? property.application_fee  : 50,
