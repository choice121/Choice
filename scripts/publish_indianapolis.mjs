...
async function main() {
    const data = JSON.parse(fs.readFileSync('scripts/indianapolis_selected.json', 'utf-8'));
    const all_to_publish = data.properties || [];

    console.log(`Loaded ${all_to_publish.length} Properties.`);
    ...
    function cleanDescription(rawDesc, address, city, state, zip_code, beds, baths, rent, prop_type) {
        ...
        const type_str = prop_type === "TOWNHOMES" ? "townhome" : "single-family home";
        const overview = `Welcome to ${address} — a beautifully maintained ${beds}-bedroom, ${baths}-bathroom ${type_str} offering exceptional comfort, modern design, and everyday convenience in ${city}, ${state} ${zip_code}.`;
...
• Attached garage and/or off-street driveway parking
• Private backyard space for outdoor living and recreation
• Dedicated laundry area with washer/dryer hookups
...
• Lease Terms: 12-Month Lease Agreement`;
    ...
    for (let i = 0; i < all_to_publish.length; i++) {
        const p = all_to_publish[i];
        ...
        if (clean_photos.length < 6) {
            console.log(`[${i+1}/${all_to_publish.length}] Skipping ${address}: only ${clean_photos.length} photos.`);
            continue;
        }
        ...
        const type_label = p.is_townhome ? "Townhome" : "Single-Family Home";
        const title = `${beds}BR/${baths}BA ${type_label} in ${city} – $${rent}/mo`;

        const amenities = [
            "Air Conditioning", "Central Heating", "Dishwasher", "Refrigerator",
            "Range / Oven", "Microwave", "Pet Friendly", "Garage/Parking",
            "Washer/Dryer Hookups", "Smoke Free"
        ];

        const payload = {
            id: prop_id, landlord_id: LANDLORD_ID, title, description: desc,
            address, city, state, zip: zip_code, county: "Marion County",
            ...
            laundry_type: "Washer/Dryer Hookups", amenities,
        ...
        if (!insertRes.ok) {
            console.log(`[${i+1}/${all_to_publish.length}] FAILED insert property ${address}: ${insertRes.status} ${await insertRes.text()}`);
            continue;
        }
        ...
        const photoRows = clean_photos.map((url, p_idx) => ({
            ...
            alt_text: `${address}, ${city} IN - Photo ${p_idx + 1}`
        }));
        ...
        console.log(`[${i+1}/${all_to_publish.length}] Published: ${address}, ${city}, ${state} ${zip_code} ($${rent}/mo | ${beds} Bed / ${baths} Bath) -> ${url}`);
    }
