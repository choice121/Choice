...
# Target rent 1400-1600
target_candidates = [c for c in all_candidates if 1400 <= c['rent'] <= 1600]
target_candidates = sorted(target_candidates, key=lambda x: x['score'], reverse=True)

# Expand to 1200-1800 if needed
expanded_candidates = [c for c in all_candidates if (1200 <= c['rent'] < 1400) or (1600 < c['rent'] <= 1800)]
# Sort by distance to the 1400-1600 range, then by score
expanded_candidates = sorted(expanded_candidates, key=lambda x: (min(abs(x['rent'] - 1400), abs(x['rent'] - 1600)), -x['score']))

seen = set()
unique_candidates = []
for c in target_candidates + expanded_candidates:
    if c['address'] not in seen:
        seen.add(c['address'])
        unique_candidates.append(c)

selected = unique_candidates[:15]
print(f"Selected: {len(selected)} (Needed 15)")

with open('scripts/indianapolis_selected.json', 'w') as f:
    json.dump({'properties': selected}, f, indent=2)
