...
async function main() {
    const data = JSON.parse(fs.readFileSync('scripts/charlotte_selected.json', 'utf-8'));
    const selected_houses = data.houses || [];
    const selected_townhomes = data.townhomes || [];
    const all_to_publish = [...selected_houses, ...selected_townhomes];

    console.log(`Loaded ${selected_houses.length} Houses and ${selected_townhomes.length} Townhomes.`);
...
• Lease Terms: 12-Month Lease Agreement`;
    ...
    for (let i = 0; i < all_to_publish.length; i++) {
        ...
        const payload = {
            ...
            address, city, state, zip: zip_code, county: "Mecklenburg County",
        ...
        const photoRows = clean_photos.map((url, p_idx) => ({
            ...
            alt_text: `${address}, ${city} NC - Photo ${p_idx + 1}`
        }));
