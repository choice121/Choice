...
def fetch_ohio_sitemap_urls():
    ...
    urls = re.findall(r'<loc>(https://www\.opendoor\.com/properties/[^<]*-OH-\d{5}/[^<]*)</loc>', xml_data, re.IGNORECASE)
    return urls

def calculate_ohio_rent(sale_price, bedrooms):
    if not sale_price:
        if bedrooms == 1: return 950
        if bedrooms == 2: return 1250
        if bedrooms == 3: return 1550
        if bedrooms == 4: return 1850
        return 2100
    
    # State-aware rent yield for Ohio: ~0.0090
    raw_rent = sale_price * 0.0090
    
    # Floor and ceilings based on bedrooms
    min_rent = 900 if bedrooms <= 2 else (1150 if bedrooms == 3 else 1350)
    max_rent = 2800 if bedrooms <= 2 else (3400 if bedrooms == 3 else 4200)
...
def clean_rental_description(raw_desc, address, city, state, zip_code, beds, baths, rent):
    ...
    overview = f"Welcome to {address} — a beautifully maintained {beds}-bedroom, {baths}-bathroom home offering exceptional comfort and convenience in {city}, {state} {zip_code}."
...
def main():
    print("=" * 70)
    print("CHOICE PROPERTIES — FULL OHIO OPENDOOR SCRAPE & PUBLISH PIPELINE")
    print("=" * 70)

    existing_addrs = get_existing_addresses()
    print(f"Found {len(existing_addrs)} existing Ohio properties in DB.")

    urls = fetch_ohio_sitemap_urls()
    print(f"Discovered {len(urls)} Ohio listings in Opendoor active sitemap.")
    ...
    for i, u in enumerate(urls, 1):
        ...
        state = (p.get('state') or 'OH').strip().upper()
        ...
        rent = calculate_ohio_rent(sale_price, beds)
        deposit = rent
        app_fee = 50
        ...
        # Minimum 6 genuine photos check
        if len(photo_urls) < 6:
        ...
        # Generate fresh UUID for property
    ...
    # Save output report
    with open("published_ohio_opendoor.json", "w") as f:
