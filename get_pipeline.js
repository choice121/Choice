async function run() {
  ...
  const res = await fetch(`${SUPABASE_URL}/rest/v1/pipeline_properties?address=in.("${addrs.join('","')}")`, { headers: { 'apikey': SUPABASE_KEY, 'Authorization': `Bearer ${SUPABASE_KEY}` } });
  
  if (res.status === 404) { console.log("No pipeline_properties table (or no permission)"); return; }
