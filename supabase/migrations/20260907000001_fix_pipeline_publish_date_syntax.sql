-- ============================================================================
-- Migration: Fix pipeline_publish date syntax error and align with platform rules
--
-- Root Cause:
-- In previous migrations, `p.listed_at <> ''` was evaluated. Because
-- `pipeline_properties.listed_at` is of type `date`, comparing `date <> text`
-- causes PostgreSQL to coerce `''` to `date` (`''::date`), resulting in:
-- "ERROR: 22007: invalid input syntax for type date: \"\"".
--
-- Fix:
-- 1. Safely handle `listed_at` using native date handling (COALESCE/IS NOT NULL).
-- 2. Safely parse `available_date` without unhandled regex/type mismatch.
-- 3. Comply with Choice Properties Directives:
--    - application_fee is always $50
--    - pets_allowed is always true
--    - smoking_allowed is false
--    - security_deposit is 1x monthly rent
--    - minimum_lease_months & lease_terms are NULL (no lease term display)
-- ============================================================================
...
BEGIN
  ...
  -- 3. Parse listed_at safely without comparing date to empty string
  ...
  INSERT INTO public.properties (
    ...
    v_avail,
    NULL, -- lease terms omitted across all properties
    NULL, -- lease terms omitted across all properties
  ...
  -- 5. Insert photos into public.property_photos
...
-- Update pipeline_publish_and_delete
...
GRANT  EXECUTE ON FUNCTION public.pipeline_publish_and_delete(text,uuid) TO authenticated, service_role, anon;
