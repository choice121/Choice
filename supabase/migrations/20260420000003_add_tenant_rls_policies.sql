...
-- Use DROP POLICY IF EXISTS followed by CREATE POLICY for idempotent migrations.
...
DROP POLICY IF EXISTS "applications_landlord_select" ON applications;
CREATE POLICY "applications_landlord_select" ON applications
  FOR SELECT
  TO authenticated
  USING (
    EXISTS (
