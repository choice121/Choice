-- Upgrade pipeline_publish to transfer ALL fields (appliances, utilities, heating, cooling, laundry, amenities, dates)
-- and automatically insert all gallery photos into public.property_photos.
...
BEGIN
  ...
  -- 4. Auto-insert photos into public.property_photos
  IF p.original_image_urls IS NOT NULL AND p.original_image_urls <> '' AND p.original_image_urls <> '[]' THEN
    BEGIN
      ...
      IF jsonb_typeof(v_photos) = 'array' THEN
        ...
        LOOP
          IF v_elem IS NOT NULL AND jsonb_typeof(v_elem) = 'string' AND length(v_elem #>> '{}') > 5 THEN
            INSERT INTO public.property_photos (
              id, property_id, url, display_order, is_hero, created_at
            ) VALUES (
              gen_random_uuid(),
              new_id,
              v_elem #>> '{}',
    ...
    EXCEPTION WHEN OTHERS THEN
      -- Proceed gracefully if photo json parsing encountered edge cases
      NULL;
    END;
  END IF;

  -- 5. Mark pipeline row as published
  ...
  RETURN json_build_object('ok', true, 'choice_property_id', new_id, 'photos_count', v_order - 1);
END;
$$;
