-- pipeline_publish v7: fix free-application regex parity with JS/Python
-- The v6 gate (20260714000001) required the literal word "application" or
-- "app" immediately before "fee" in the bare "no fee" / "zero fee" checks,
-- e.g. it caught "no application fee" but NOT "no fee to apply". The
-- client-side check (js/admin/pipeline.js validateForPublish) and the
-- Python enrichment engine (scraper/enrichment.py _FREE_APP_RE) already
-- treat "application"/"app" as optional in that position. This migration
-- brings the SQL gate back into parity so every publish path — admin UI,
-- direct RPC call, or the iOS Scriptable import path (which skips Python
-- enrichment entirely) — rejects the same set of phrasings.
...
BEGIN
  ...
  -- "application"/"app" is OPTIONAL before "fee" here (matches JS/Python):
  -- catches "no fee to apply", "zero fee", etc. — not just "no application fee".
  IF p.description IS NOT NULL AND p.description ~* (
    'free\s+application'
    '|apply\s+for\s+free'
    '|no\s+(?:application\s+|app\s+)?fee'
    '|\$\s*0\.?0*\s+(?:application\s+|app\s+)?fee'
    '|zero\s+(?:application\s+|app\s+)?fee'
