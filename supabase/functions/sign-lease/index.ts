import { createClient } from 'npm:@supabase/supabase-js@2';
  ...
  Deno.serve(async (req: Request) => {
    ...
    // Identity pre-check by token (tenant token only - co-applicant flow
    // uses a separate RPC and is not changed by Phase 04).
    ...
    // Mark every attached addendum (still un-acknowledged) as acknowledged
    // by this tenant signing. Non-fatal if ack persistence fails.
    try {
    ...
    if (appSigned) {
      // Re-render PDF using the pinned template snapshot (Phase 02) plus
      // the per-application addenda snapshots (Phase 04).
