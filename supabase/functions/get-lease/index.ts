import { createClient } from 'npm:@supabase/supabase-js@2';
  ...
  //
  // Phase 04 - also returns the per-application addenda snapshots so the
  // signing UI can render a checkbox per addendum and gate the Sign
  // button until all have been individually acknowledged.
  Deno.serve(async (req: Request) => {
    ...
    // 5. Phase 04 - load attached addenda for the signing UI. We intentionally
    //    only return body-relevant fields (slug/title/jurisdiction/citation/
    //    rendered_body/signature/initials flags). PII like the previous
    //    signer's IP/user-agent stays server-side.
