  ...
  return (
    <main className="min-h-screen bg-slate-950 text-slate-100">
      <div className="mx-auto max-w-7xl px-6 py-10 lg:px-8">
        <header className="mb-10 rounded-[28px] border border-slate-800 bg-slate-900/75 p-6 shadow-[0_30px_80px_rgba(2,6,23,0.8)] backdrop-blur">
        ...
        <section className="grid gap-8 lg:grid-cols-[1.15fr_0.85fr]">
          <div className="rounded-[24px] border border-slate-800 bg-slate-900 p-6">
            <div className="mb-5 flex items-center justify-between gap-4">
              <div>
                <p className="text-xs uppercase tracking-[0.18em] text-slate-400">Protected flow</p>
            ...
            {isSubmitted ? (
              <div className="rounded-2xl border border-emerald-500/30 bg-emerald-500/10 p-5 text-emerald-100">
                <p className="text-xs uppercase tracking-[0.18em] text-emerald-300">Submission status</p>
              ...
              <div className="space-y-6">
                  ...
                  <div className="space-y-4">
                    <div className="grid gap-4 md:grid-cols-2">
                      <label className="block">
                        <span className="mb-2 block text-xs uppercase tracking-[0.14em] text-slate-400">First name</span>
                        <input
                      ...
                      <label className="block">
                        <span className="mb-2 block text-xs uppercase tracking-[0.14em] text-slate-400">Last name</span>
                        <input
                          value={form.lastName}
                    ...
                    <div className="grid gap-4 md:grid-cols-2">
                      ...
                      <label className="block">
                        <span className="mb-2 block text-xs uppercase tracking-[0.14em] text-slate-400">Phone</span>
                        <input
                          value={form.phone}
                    ...
                    <label className="block">
                      <span className="mb-2 block text-xs uppercase tracking-[0.14em] text-slate-400">Date of birth</span>
                      <input
                        type="date"
                        value={form.dob}
                ...
                {step === 2 && (
                  <div className="space-y-4">
                    <div className="rounded-2xl border border-slate-800 bg-slate-950/60 p-4">
                      <p className="mb-3 text-xs uppercase tracking-[0.14em] text-slate-400">Current housing history</p>
                      <div className="grid gap-4 md:grid-cols-2">
                        <label className="block md:col-span-2">
                          <span className="mb-2 block text-xs uppercase tracking-[0.14em] text-slate-400">Current address</span>
                          <input
                        ...
                        <label className="block">
                          <span className="mb-2 block text-xs uppercase tracking-[0.14em] text-slate-400">Monthly rent</span>
                        ...
                        <label className="block">
                          <span className="mb-2 block text-xs uppercase tracking-[0.14em] text-slate-400">Landlord name</span>
                    ...
                    <div className="rounded-2xl border border-slate-800 bg-slate-950/60 p-4">
                      <p className="mb-3 text-xs uppercase tracking-[0.14em] text-slate-400">Employment & income</p>
                      <div className="grid gap-4 md:grid-cols-2">
                        <label className="block">
                          <span className="mb-2 block text-xs uppercase tracking-[0.14em] text-slate-400">Employment status</span>
                        ...
                        <label className="block">
                          <span className="mb-2 block text-xs uppercase tracking-[0.14em] text-slate-400">Monthly income</span>
                ...
                {step === 3 && (
                  <div className="space-y-4">
                    <div className="rounded-2xl border border-slate-800 bg-slate-950/60 p-4">
                      <p className="mb-3 text-xs uppercase tracking-[0.14em] text-slate-400">Consent & submission</p>
                    ...
                    <div className="rounded-2xl border border-slate-800 bg-slate-950/60 p-4">
                      <p className="mb-3 text-xs uppercase tracking-[0.14em] text-slate-400">Protected backend contract</p>
          ...
          <div className="space-y-8">
              ...
              <ul className="space-y-3">
                {contractGuardrails.map((item) => (
                  <li key={item} className="flex gap-3 rounded-2xl border border-slate-800 bg-slate-950/60 p-3 text-sm text-slate-200">
                    <span className="mt-1 inline-block h-2.5 w-2.5 rounded-full bg-emerald-400" aria-hidden="true" />
                    <span>{item}</span>
                  </li>
                ))}
              </ul>
            ...
            <div className="rounded-[24px] border border-slate-800 bg-slate-900 p-6">
              <h2 className="mb-5 text-xl font-semibold text-white">Protected flow</h2>
              <div className="space-y-4">
                {protectedFlow.map((stepName, index) => (
                  <div key={stepName} className="flex items-center gap-3 rounded-2xl border border-slate-800 bg-slate-950/60 p-3">
                    <div className="flex h-8 w-8 items-center justify-center rounded-full bg-cyan-500/10 text-xs font-semibold text-cyan-200">
                      {index + 1}
                    </div>
            ...
            <div className="rounded-[24px] border border-slate-800 bg-slate-900 p-6">
              <h2 className="mb-5 text-xl font-semibold text-white">Status model</h2>
              <div className="flex flex-wrap gap-2">
                {statusStates.map((state) => (
                  <div key={state.label} className={`inline-flex items-center rounded-full border px-3 py-1.5 text-sm font-medium ${state.tone}`}>
                    {state.label}
                  </div>
                ))}
              </div>
        ...
        <section className="mt-8 rounded-[24px] border border-slate-800 bg-slate-900 p-6">
          ...
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            {migrationPhases.map((phase) => (
              <div key={phase.name} className="rounded-2xl border border-slate-800 bg-slate-950/60 p-4">
                <p className="text-[10px] uppercase tracking-[0.18em] text-cyan-300">{phase.name}</p>
