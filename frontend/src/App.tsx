...
function App() {
  ...
  const getPropsStatusClass = () => {
    ...
    return 'rounded-full border border-zillow-green/40 bg-zillow-green/10 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.14em] text-zillow-green-dark'
  }
  ...
  const getAuthStatusClass = () => {
    ...
    if (isAuthenticated) return 'rounded-full border border-zillow-green/40 bg-zillow-green/10 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.14em] text-zillow-green-dark'
      ...
      return (
        <label className="flex items-start gap-3 rounded-2xl border border-slate-300 bg-slate-50/60 p-3 text-sm text-slate-700">
          <input
            ...
            className="mt-1 h-4 w-4 accent-zillow-blue"
          />
          <span>
    ...
    return (
      <label className="block">
        ...
        <input
          ...
          className={`w-full rounded-xl border bg-slate-50 px-3 py-2.5 text-slate-50 outline-none transition focus:border-zillow-blue ${error ? 'border-rose-500/70' : 'border-slate-300'}`}
        />
      ...
      <div className="mx-auto max-w-7xl px-5 py-8 sm:px-6 lg:px-8 lg:py-10">
        <header className="mb-8 rounded-[28px] border border-slate-200 bg-white/75 p-5 shadow-[0_24px_80px_rgba(2,6,23,0.7)] backdrop-blur sm:p-6">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <p className="mb-3 text-[11px] font-semibold uppercase tracking-[0.24em] text-zillow-blue">
            ...
            <div className="rounded-2xl border border-zillow-green/30 bg-zillow-green/10 px-4 py-3 text-sm text-zillow-green-dark">
        ...
        <section className="grid gap-8 lg:grid-cols-[1.12fr_0.88fr]">
          <div className="rounded-[24px] border border-slate-200 bg-white p-5 sm:p-6">
            <div className="mb-5 flex items-center justify-between gap-4">
              ...
              <span className="rounded-full border border-zillow-blue/40 bg-zillow-blue/10 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.18em] text-cyan-200">
                {stepIndex + 1} / {stepOrder.length}
              </span>
            </div>

            <div className="mb-6 h-2 overflow-hidden rounded-full bg-slate-100">
              <div className="h-full rounded-full bg-zillow-blue transition-all duration-300" style={{ width: `${progress}%` }} />
            </div>
              ...
              {stepOrder.map((step, index) => (
                <button
                  ...
                  className={`rounded-full border px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.12em] ${
                    index === stepIndex
                      ? 'border-zillow-blue/40 bg-zillow-blue/10 text-cyan-200'
            ...
            <div className="mt-6 flex items-center justify-between gap-3 border-t border-slate-200 pt-5">
                ...
                {stepIndex < stepOrder.length - 1 ? (
                  <button
                    type="button"
                    onClick={goNext}
                    className="rounded-xl bg-zillow-blue px-4 py-2 text-sm font-semibold text-white shadow-lg  transition hover:bg-zillow-blue-dark"
                  >
                    Continue
                  </button>
                ) : (
                  <button
                    ...
                    className="rounded-xl bg-gradient-to-r from-emerald-500 to-teal-500 px-4 py-2 text-sm font-semibold text-slate-900 shadow-lg shadow-emerald-900/30 transition hover:bg-zillow-blue-dark"
                  >
            ...
            <div className="rounded-[24px] border border-slate-200 bg-white p-6">
              ...
              <div className="mt-5 space-y-4">
                ...
                {isAuthenticated && user && (
                  <div className="rounded-xl border border-zillow-green/30 bg-zillow-green/10 p-3 text-sm text-zillow-green-dark">
                ...
                {migrationPhases.map((phase) => (
                  <div key={phase.name} className="rounded-xl border border-slate-200 bg-slate-50/60 p-3">
                    <div className="flex items-center justify-between gap-3">
                      <span className="text-sm font-semibold text-zillow-blue">{phase.name}</span>
                  ...
                  {routeMigrationMap.map((item) => (
                    <tr key={item.route}>
                      ...
                      <td className="px-3 py-2">
                        <span className="inline-flex rounded-full border border-zillow-blue/30 bg-zillow-blue/10 px-2 py-1 text-[10px] font-semibold uppercase tracking-[0.12em] text-cyan-200">
                          {item.status}
                        </span>
                      </td>
                    </tr>
            ...
            <ul className="mt-5 space-y-3">
              {authProtections.map((item) => (
                <li key={item} className="flex items-start gap-3 rounded-xl border border-slate-200 bg-slate-50/60 p-3 text-sm text-slate-700">
                  <span className="mt-0.5 inline-flex h-5 w-5 items-center justify-center rounded-full bg-zillow-green/15 text-[10px] text-zillow-green-dark">✓</span>
                  <span>{item}</span>
                </li>
              ))}
            </ul>
            ...
            <div className="mt-5 space-y-3">
              {protectedFlow.map((stepName, index) => (
                <div key={stepName} className="flex items-center gap-3 rounded-xl border border-slate-200 bg-slate-50/60 p-3">
                  <div className="flex h-8 w-8 items-center justify-center rounded-full bg-zillow-blue/15 text-sm font-semibold text-cyan-200">
                    {index + 1}
                  </div>
                ...
                <tbody className="divide-y divide-slate-800 bg-white">
                  {regressionMatrix.map((row) => (
                    <tr key={row.step}>
                      ...
                      <td className="px-3 py-2">
                        <span className="inline-flex rounded-full border border-zillow-green/30 bg-zillow-green/10 px-2 py-1 text-xs font-medium text-zillow-green-dark">
                          {row.mustPreserve}
                        </span>
                      </td>
                    </tr>
