          ...
          {properties.map((property) => (
            ...
            >
              ...
              >
                ...
                <button type="button" aria-label={savedIds.has(property.id) ? `Remove ${property.title || property.address} from saved homes` : `Save ${property.title || property.address}`} aria-pressed={savedIds.has(property.id)} onClick={(e) => { e.stopPropagation(); toggleSaved(property.id); }} className={`absolute top-3 right-3 p-2 rounded-full backdrop-blur-md shadow-sm border ${savedIds.has(property.id) ? 'bg-rose-50 border-rose-200 text-rose-500' : 'bg-white/90 border-slate-200 text-slate-400 hover:text-slate-900'} hover:scale-110 transition z-10`}><svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill={savedIds.has(property.id) ? "currentColor" : "none"} stroke="currentColor"><path fillRule="evenodd" d="M3.172 5.172a4 4 0 015.656 0L10 6.343l1.172-1.171a4 4 0 115.656 5.656L10 17.657l-6.828-6.829a4 4 0 010-5.656z" clipRule="evenodd" /></svg></button>
                {/* Badges overlay */}
