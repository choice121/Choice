...
const config = {
  SUPABASE_URL:      process.env.SUPABASE_URL      || 'https://tlfmwetmhthpyrytrcfo.supabase.co',
  ...
  IMAGEKIT_PUBLIC_KEY: process.env.IMAGEKIT_PUBLIC_KEY || 'public_21rg7lvzo',
...
if (missing.length) {
  ...
  console.warn('   Using defaults or empty values for development build.');
}
...
if (config.SUPABASE_URL) {
  try {
    ...
    if (!supabaseHost.startsWith(EXPECTED_SUPABASE_PROJECT_REF + '.')) {
      console.warn('⚠️ SUPABASE_URL project ref mismatch: expected', EXPECTED_SUPABASE_PROJECT_REF, 'got', supabaseHost);
    }
  } catch {
...
if (!config.GEOAPIFY_API_KEY) {
  console.warn('⚠️ GEOAPIFY_API_KEY is not set — address autocomplete will be disabled');
}
      ...
      await new Promise(function(resolve) {
        const req = https.request({
          ...
          if (res.statusCode === 200) {
            ...
            console.warn('⚠️ Supabase returned status', res.statusCode);
          }
          resolve();
        });
        req.on('timeout', function() {
          req.destroy();
          console.warn('⚠️ Supabase credential check timed out.');
          resolve();
        });
        req.on('error', function(err) {
          console.warn('⚠️ Supabase credential check network error:', err.message);
          resolve();
        });
        req.end();
      });
    } catch (err) {
      console.warn('⚠️ Supabase credential check threw an unexpected error:', err.message);
    }
  })();
}
...
await (async function injectInitialListings() {
  ...
  if (!supabaseUrl) {
    console.warn('⚠  SUPABASE_URL not configured — skipping property pre-load');
    return;
  }
  ...
  const data = await new Promise(function(resolve) {
    let urlObj;
    try {
      urlObj = new URL(supabaseUrl + apiPath);
    } catch (e) {
      console.warn('⚠  Invalid Supabase URL — skipping property pre-load:', e.message);
      resolve(null);
      return;
    }
    const req = https.request({
