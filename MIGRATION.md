# MIGRATION.md ÃÂ¢ÃÂÃÂ Choice Properties: GAS ÃÂ¢ÃÂÃÂ Supabase Migration
## Living Project Guide ÃÂ¢ÃÂÃÂ Read This First
...
## THE GITHUB COMMIT RULE ÃÂ¢ÃÂÃÂ Non-Negotiable

**Every AI working on this project must follow this rule without exception:**

1. **After every successful fix or completed feature** ÃÂ¢ÃÂÃÂ commit and push to `choice121/Choice` on GitHub before moving to the next task. Do not batch multiple fixes into one push. One fix = one push.

2. **After completing any phase** ÃÂ¢ÃÂÃÂ update the Phase Tracker in this file (Section 4) to mark the phase as Complete, then push the updated `MIGRATION.md` to GitHub before starting the next phase.

3. **If a fix fails** ÃÂ¢ÃÂÃÂ do not push. Fix it first, verify it works, then push.
...
4. **How to push via GitHub API** (use this pattern from any platform):
   ...
   > token value ÃÂ¢ÃÂÃÂ it is not stored in this file for security reasons.

5. **Commit message format:**
   ```
   [Phase X] Short description of what was done
   Examples:
     [Phase 0] Run SETUP.sql ÃÂ¢ÃÂÃÂ applications table verified in Supabase
     [Phase 1] Wire admin/applications.html to live Supabase table
     [Phase 1] Add serve.js /generate-lease pdfkit endpoint
     [Phase 1] Complete ÃÂ¢ÃÂÃÂ all admin pages live, e-signing working
   ```
...
- `admin_roles` ÃÂ¢ÃÂÃÂ admin user IDs
- `landlords` ÃÂ¢ÃÂÃÂ landlord profiles linked to `auth.users`
- `properties` ÃÂ¢ÃÂÃÂ property listings linked to landlords
- `inquiries` ÃÂ¢ÃÂÃÂ tenant contact inquiries
- `messages` ÃÂ¢ÃÂÃÂ in-thread messages per application (app_id references)
- `email_logs` ÃÂ¢ÃÂÃÂ email send audit trail
- `saved_properties` ÃÂ¢ÃÂÃÂ tenant saved listings
- `admin_actions` ÃÂ¢ÃÂÃÂ admin audit log
- `rate_limit_log` ÃÂ¢ÃÂÃÂ rate limiting
- `pipeline_properties`, `pipeline_scrape_runs`, `pipeline_enrichment_log` ÃÂ¢ÃÂÃÂ property pipeline
...
- A complete `applications` table with ~80 columns covering applicant data, lease terms, signing tokens, move-in status, payment status ÃÂ¢ÃÂÃÂ all consolidated into one table
...
### 2c. GAS Backend ÃÂ¢ÃÂÃÂ What It Actually Does
...
**Section 1 ÃÂ¢ÃÂÃÂ Applicant Identity:**
...
**Section 2 ÃÂ¢ÃÂÃÂ Residence & Household:**
...
**Section 6 ÃÂ¢ÃÂÃÂ Background & Consent:**
...
- `CP.sb()` ÃÂ¢ÃÂÃÂ the lazy Supabase singleton (from `cp-api.js`)
...
The actual SETUP.sql ÃÂ¢ÃÂÃÂ the authoritative schema file already committed to the main repo ÃÂ¢ÃÂÃÂ takes a **consolidated approach**: all lease, signing, move-in, and payment data lives as columns on the `applications` table, with `co_applicants` as the only satellite table.
...
3. The GAS admin panel treats applications and their lease status as one unit ÃÂ¢ÃÂÃÂ keeping them together preserves this mental model
...
- A `sign_events` or `signature_log` table for the full e-sign audit trail (typed name, IP, user-agent, timestamp per signing event) ÃÂ¢ÃÂÃÂ this is distinct from just storing the signature on the application row
- A `move_ins` standalone table (currently move-in data is columns on `applications` ÃÂ¢ÃÂÃÂ this works for MVP but may warrant extraction later)
- Supabase Storage buckets: `lease-pdfs`, `application-docs` (may already exist ÃÂ¢ÃÂÃÂ see section 15 of SETUP.sql)
...
| Phase | Name | Status |
  |-------|------|--------|
  | Phase 0 | Apply SETUP.sql to live database | **COMPLETE** â applications table and all schema verified live in Supabase |
  | Phase 1 | Build native admin system (GAS untouched) | **COMPLETE** â admin/applications.html, leases.html, move-ins.html, dashboard.html, cp-api.js, lease-sign.html all live. Last updated: 2026-04-18 |
  | Phase 2 | Tenant Portal (magic link auth + self-service) | **COMPLETE** â tenant/login.html + tenant/portal.html built. Magic link OTP auth, status timeline, lease download, move-in details, message queries. Last updated: 2026-04-18 |
  | Phase 3 | Applications Pipeline (email on status change) | **COMPLETE** â admin/applications.html: sendStatusEmail() on approve/deny via send-email edge function, toast notifications. Last updated: 2026-04-18 |
  ...
  | Phase 11 | Archive GAS | **COMPLETE** — MIGRATION_new_columns.sql applied 2026-04-18 (15 new columns confirmed live). End-to-end test passed. BACKEND_URL confirmed on CF Pages. Full admin flow tested: status transitions, holding fee, payment, countersign, move-in scheduling — all passed. send-email Edge Function bug fixed (`.catch()` on Supabase insert replaced with try/catch; verify_jwt set to false to support ES256 admin JWTs — function does its own admin_roles check). send-email re-deployed via Supabase CLI (v3 active). Verified: authenticated sends work (HTTP 200), unauthenticated calls correctly rejected (HTTP 401). 9 QA test apps cleaned from DB. Remaining manual step: disable GAS doPost handler. Last updated: 2026-04-18 |
...
Steps:
1. Open Supabase Dashboard ÃÂ¢ÃÂÃÂ SQL Editor
2. Paste SETUP.sql in full
3. Run it (safe to re-run ÃÂ¢ÃÂÃÂ uses `IF NOT EXISTS`, `OR REPLACE`, `ON CONFLICT`)
...
### Phase 1 ÃÂ¢ÃÂÃÂ Build in Parallel (GAS Untouched)

**What to build:**
1. **`/admin/applications.html`** ÃÂ¢ÃÂÃÂ Full application list and detail view pulling from `applications` table. Status badges, filter by status, expand detail panel, admin actions (approve/deny/shortlist/mark-under-review) with `admin_notes`. Landlord view scoped by RLS.
2. **`/admin/leases.html`** ÃÂ¢ÃÂÃÂ Lease management: generate lease (calls `/generate-lease` endpoint on serve.js which uses pdfkit), send signing email, track signing status, view/download PDF from Supabase Storage.
3. **`/admin/move-ins.html`** ÃÂ¢ÃÂÃÂ Move-in management: confirm move-in, set move-in date, update move-in status.
4. **`serve.js` endpoint** ÃÂ¢ÃÂÃÂ Add `/generate-lease` POST endpoint that accepts lease data JSON, generates PDF using pdfkit, uploads to Supabase Storage, updates `applications.lease_pdf_url`.
5. **Lease template** ÃÂ¢ÃÂÃÂ A JS/JSON structure (`/lease-template.js` or `/admin/js/lease-template.js`) with all clause variables.
6. **E-signing pages** ÃÂ¢ÃÂÃÂ A new public page `/lease-sign.html` (or served via serve.js) for tenant signing flow.
7. **Email sending** ÃÂ¢ÃÂÃÂ Gmail SMTP via Nodemailer in serve.js for: signing invitations, approval/denial notices, move-in confirmations.
8. **`/admin/dashboard.html`** ÃÂ¢ÃÂÃÂ Wire up stat cards (pending, under review, approved, denied, lease sent, lease signed, move-in) from live `applications` table.
...
| All admin decisions are manual | Admins and landlords make all approve/deny/shortlist decisions ÃÂ¢ÃÂÃÂ no automation |
| serve.js stays as pure static server + new endpoints | Do not convert to Express ÃÂ¢ÃÂÃÂ serve.js is a minimal `http.createServer` ÃÂ¢ÃÂÃÂ add endpoints as route handlers following existing pattern |
...
1. **Phase 0** ÃÂ¢ÃÂÃÂ Run SETUP.sql against live Supabase (prerequisite for everything)
2. **Lease template structure** ÃÂ¢ÃÂÃÂ Define the JS template first (needed before PDF generation)
3. **serve.js lease PDF endpoint** ÃÂ¢ÃÂÃÂ Add `/generate-lease` handler using pdfkit
4. **`/admin/applications.html`** ÃÂ¢ÃÂÃÂ Wire to live `applications` table; add manual action buttons (approve/deny/shortlist/under-review); add admin_notes field; add landlord-scoped view
5. **`/admin/dashboard.html`** ÃÂ¢ÃÂÃÂ Wire stat cards using same `applications` table queries
6. **E-sign page** (`/lease-sign.html`) ÃÂ¢ÃÂÃÂ Public-facing page for tenants to sign
7. **`/admin/leases.html`** ÃÂ¢ÃÂÃÂ Generate, send, track leases; download PDF
8. **`/admin/move-ins.html`** ÃÂ¢ÃÂÃÂ Confirm move-ins, update status
9. **Email wiring** ÃÂ¢ÃÂÃÂ Add Nodemailer SMTP to serve.js for all notification types
10. **Landlord portal** ÃÂ¢ÃÂÃÂ Add applications section to landlord portal pages, scoped by RLS
...
- Applicant authenticates via Supabase Auth (email OTP ÃÂ¢ÃÂÃÂ already enabled per SETUP.sql comments)
...
- If lease is in `sent` state: show "Sign Your Lease" button ÃÂ¢ÃÂÃÂ links to `/lease-sign.html?token=<tenant_sign_token>`
- The `messages` table (already exists in live DB) handles back-and-forth communication between admin/tenant

**This is a Phase 1 deliverable** ÃÂ¢ÃÂÃÂ it must exist before Phase 2 cutover so tenants have a place to check status on the new system.

---

## 8. Landlord Access ÃÂ¢ÃÂÃÂ Requirements
...
- Approve / Shortlist / Decline buttons (write back to `status` field ÃÂ¢ÃÂÃÂ but the landlord needs UPDATE permission)
...
1. **SSN data** ÃÂ¢ÃÂÃÂ applications.ssn stores only last-4 digits (masked by Edge Function). Never expose full SSN through any new endpoint.
2. **lease_sign_token** ÃÂ¢ÃÂÃÂ One-time signing token must be invalidated after use (set to NULL or a used flag after signing).
3. **pdfkit endpoint** ÃÂ¢ÃÂÃÂ The `/generate-lease` endpoint in serve.js must be admin-only (verify Supabase session server-side before generating).
4. **Document uploads** ÃÂ¢ÃÂÃÂ Supabase Storage bucket `application-docs` must have RLS restricting access to admin, the owning applicant, and the relevant landlord.
5. **CORS** ÃÂ¢ÃÂÃÂ The new serve.js endpoints will be called from admin pages on the same origin ÃÂ¢ÃÂÃÂ no CORS issues expected. The lease signing page calls serve.js from a different origin ÃÂ¢ÃÂÃÂ CORS headers needed.

---

## 10. Phase 2 Cutover Risks

1. **GAS field name mapping vs. Supabase column names** ÃÂ¢ÃÂÃÂ GAS uses "Title Case with spaces" keys; Supabase uses `snake_case`. The import script must map these precisely.
2. **Missing property_id linkage** ÃÂ¢ÃÂÃÂ GAS applications may not have a valid `property_id` that matches a Supabase property ID (GAS stores the ID from URL params, which could be stale or edited). A manual reconciliation step may be needed.
3. **SSN data** ÃÂ¢ÃÂÃÂ GAS stores masked SSN (last 4). Do not re-mask during import.
4. **Document URLs** ÃÂ¢ÃÂÃÂ GAS stores Google Drive URLs. These are inaccessible to Supabase Storage. Import should store these Drive URLs in a `legacy_doc_url` column (add it to applications as part of Phase 2 prep).
5. **Timing** ÃÂ¢ÃÂÃÂ During cutover, a window exists where the old URL is disabled and the new URL is not yet tested. Use a feature flag (env var in Cloudflare Pages) to flip atomically.
6. **Duplicate applications** ÃÂ¢ÃÂÃÂ Applications submitted during the cutover window could be missed if the GAS URL is shut down before the new endpoint is live. Keep GAS running as fallback for 48 hours after cutover.
