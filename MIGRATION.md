# MIGRATION.md â Choice Properties: GAS â Supabase Migration
## Living Project Guide â Read This First

---

> **FOR ANY AI BEGINNING WORK ON THIS PROJECT:**
> Before writing a single line of code, you MUST read:
> 1. This file (`MIGRATION.md`) â the project context and phase tracker
> 2. `MIGRATION_SCHEMA.sql` â the authoritative SQL for all tables and policies
> 3. `MIGRATION_PATTERNS.md` â every coding convention you must follow
>
...
> Do not trust your own assumptions â trust these files.

---

## THE GITHUB COMMIT RULE â Non-Negotiable

**Every AI working on this project must follow this rule without exception:**

1. **After every successful fix or completed feature** â commit and push to `choice121/Choice` on GitHub before moving to the next task. Do not batch multiple fixes into one push. One fix = one push.

2. **After completing any phase** â update the Phase Tracker in this file (Section 4) to mark the phase as Complete, then push the updated `MIGRATION.md` to GitHub before starting the next phase.

3. **If a fix fails** â do not push. Fix it first, verify it works, then push.
...
4. **How to push via GitHub API** (use this pattern from any platform):
   ...
   > token value â it is not stored in this file for security reasons.

5. **Commit message format:**
   ```
   [Phase X] Short description of what was done
   Examples:
     [Phase 0] Run SETUP.sql â applications table verified in Supabase
     [Phase 1] Wire admin/applications.html to live Supabase table
     [Phase 1] Add serve.js /generate-lease pdfkit endpoint
     [Phase 1] Complete â all admin pages live, e-signing working
   ```
...
| GAS backend | Google Apps Script | Google Sheets as DB, admin UI via GAS HTML | Live â being replaced |

**The migration goal:** Replace the GAS/Google Sheets pipeline with a native Supabase system embedded in the existing admin panel â while keeping GAS fully operational until the new system is confirmed working.
...
- `admin_roles` â admin user IDs
- `landlords` â landlord profiles linked to `auth.users`
- `properties` â property listings linked to landlords
- `inquiries` â tenant contact inquiries
- `messages` â in-thread messages per application (app_id references)
- `email_logs` â email send audit trail
- `saved_properties` â tenant saved listings
- `admin_actions` â admin audit log
- `rate_limit_log` â rate limiting
- `pipeline_properties`, `pipeline_scrape_runs`, `pipeline_enrichment_log` â property pipeline
...
- A complete `applications` table with ~80 columns covering applicant data, lease terms, signing tokens, move-in status, payment status â all consolidated into one table
...
### 2c. GAS Backend â What It Actually Does
...
### 2d. Application Form Fields â Complete List
...
**Section 1 â Applicant Identity:**
...
**Section 2 â Residence & Household:**
...
**Section 6 â Background & Consent:**
...
- `CP.sb()` â the lazy Supabase singleton (from `cp-api.js`)
...
The actual SETUP.sql â the authoritative schema file already committed to the main repo â takes a **consolidated approach**: all lease, signing, move-in, and payment data lives as columns on the `applications` table, with `co_applicants` as the only satellite table.
...
3. The GAS admin panel treats applications and their lease status as one unit â keeping them together preserves this mental model
...
- A `sign_events` or `signature_log` table for the full e-sign audit trail (typed name, IP, user-agent, timestamp per signing event) â this is distinct from just storing the signature on the application row
- A `move_ins` standalone table (currently move-in data is columns on `applications` â this works for MVP but may warrant extraction later)
- Supabase Storage buckets: `lease-pdfs`, `application-docs` (may already exist â see section 15 of SETUP.sql)
...
| Phase | Name | Status |
|-------|------|--------|
| Phase 0 | Apply SETUP.sql to live database | **Pending confirmation** â SETUP.sql must be manually run in Supabase SQL Editor before Phase 1 pages will work |
| Phase 1 | Build native system in admin panel (GAS untouched) | **IN PROGRESS** â admin pages built (applications, leases, move-ins, dashboard, cp-api.js), serve.js endpoints added (generate-lease, get-lease, sign-lease, send-email, download-lease), lease-sign.html built. Last updated: 2026-04-18 |
| Phase 2 | Cutover (change apply form endpoint) | NOT STARTED |
| Phase 3 | Archive GAS | NOT STARTED |

### Phase 0 â Apply SETUP.sql (Prerequisite)
...
3. Run it (safe to re-run â uses `IF NOT EXISTS`, `OR REPLACE`, `ON CONFLICT`)
...
### Phase 1 â Build in Parallel (GAS Untouched)

**What to build:**
1. **`/admin/applications.html`** â Full application list and detail view pulling from `applications` table. Status badges, filter by status, expand detail panel, admin actions (approve/deny/shortlist/mark-under-review) with `admin_notes`. Landlord view scoped by RLS.
2. **`/admin/leases.html`** â Lease management: generate lease (calls `/generate-lease` endpoint on serve.js which uses pdfkit), send signing email, track signing status, view/download PDF from Supabase Storage.
3. **`/admin/move-ins.html`** â Move-in management: confirm move-in, set move-in date, update move-in status.
4. **`serve.js` endpoint** â Add `/generate-lease` POST endpoint that accepts lease data JSON, generates PDF using pdfkit, uploads to Supabase Storage, updates `applications.lease_pdf_url`.
5. **Lease template** â A JS/JSON structure (`/lease-template.js` or `/admin/js/lease-template.js`) with all clause variables.
6. **E-signing pages** â A new public page `/lease-sign.html` (or served via serve.js) for tenant signing flow.
7. **Email sending** â Gmail SMTP via Nodemailer in serve.js for: signing invitations, approval/denial notices, move-in confirmations.
8. **`/admin/dashboard.html`** â Wire up stat cards (pending, under review, approved, denied, lease sent, lease signed, move-in) from live `applications` table.
...
2. Import into `applications` table (field mapping guide needed â see MIGRATION_PATTERNS.md)
...
| All admin decisions are manual | Admins and landlords make all approve/deny/shortlist decisions â no automation |
| serve.js stays as pure static server + new endpoints | Do not convert to Express â serve.js is a minimal `http.createServer` â add endpoints as route handlers following existing pattern |
...
1. **Phase 0** â Run SETUP.sql against live Supabase (prerequisite for everything)
2. **Lease template structure** â Define the JS template first (needed before PDF generation)
3. **serve.js lease PDF endpoint** â Add `/generate-lease` handler using pdfkit
4. **`/admin/applications.html`** â Wire to live `applications` table; add manual action buttons (approve/deny/shortlist/under-review); add admin_notes field; add landlord-scoped view
5. **`/admin/dashboard.html`** â Wire stat cards using same `applications` table queries
6. **E-sign page** (`/lease-sign.html`) â Public-facing page for tenants to sign
7. **`/admin/leases.html`** â Generate, send, track leases; download PDF
8. **`/admin/move-ins.html`** â Confirm move-ins, update status
9. **Email wiring** â Add Nodemailer SMTP to serve.js for all notification types
10. **Landlord portal** â Add applications section to landlord portal pages, scoped by RLS
...
- Applicant authenticates via Supabase Auth (email OTP â already enabled per SETUP.sql comments)
...
- If lease is in `sent` state: show "Sign Your Lease" button â links to `/lease-sign.html?token=<tenant_sign_token>`
- The `messages` table (already exists in live DB) handles back-and-forth communication between admin/tenant

**This is a Phase 1 deliverable** â it must exist before Phase 2 cutover so tenants have a place to check status on the new system.

---

## 8. Landlord Access â Requirements
...
- Approve / Shortlist / Decline buttons (write back to `status` field â but the landlord needs UPDATE permission)
...
1. **SSN data** â applications.ssn stores only last-4 digits (masked by Edge Function). Never expose full SSN through any new endpoint.
2. **lease_sign_token** â One-time signing token must be invalidated after use (set to NULL or a used flag after signing).
3. **pdfkit endpoint** â The `/generate-lease` endpoint in serve.js must be admin-only (verify Supabase session server-side before generating).
4. **Document uploads** â Supabase Storage bucket `application-docs` must have RLS restricting access to admin, the owning applicant, and the relevant landlord.
5. **CORS** â The new serve.js endpoints will be called from admin pages on the same origin â no CORS issues expected. The lease signing page calls serve.js from a different origin â CORS headers needed.

---

## 10. Phase 2 Cutover Risks

1. **GAS field name mapping vs. Supabase column names** â GAS uses "Title Case with spaces" keys; Supabase uses `snake_case`. The import script must map these precisely.
2. **Missing property_id linkage** â GAS applications may not have a valid `property_id` that matches a Supabase property ID (GAS stores the ID from URL params, which could be stale or edited). A manual reconciliation step may be needed.
3. **SSN data** â GAS stores masked SSN (last 4). Do not re-mask during import.
4. **Document URLs** â GAS stores Google Drive URLs. These are inaccessible to Supabase Storage. Import should store these Drive URLs in a `legacy_doc_url` column (add it to applications as part of Phase 2 prep).
5. **Timing** â During cutover, a window exists where the old URL is disabled and the new URL is not yet tested. Use a feature flag (env var in Cloudflare Pages) to flip atomically.
6. **Duplicate applications** â Applications submitted during the cutover window could be missed if the GAS URL is shut down before the new endpoint is live. Keep GAS running as fallback for 48 hours after cutover.
